...
def connect_bridge(ip: Optional[str] = None) -> Bridge:
    b = Bridge(ip) if ip else Bridge() # Create a Bridge object. If ip is None, phue will try to discover it on the LAN.
    return b

def cmd_pair(ip: Optional[str]) -> None: #Register this script with the bridge. Press the physical link button, then run: python hue_test.py --pair
    try:
        b = connect_bridge(ip)
...
def cmd_list_groups(ip: Optional[str]) -> None: # Print available groups/zones so you can use the correct names with --group.
    try:
...
def cmd_set_group_hue(ip: Optional[str], name: str, hue_value: int, use_degrees: bool) -> None: #Set a group's color using Hue "hue" attribute.
    try:
        b = connect_bridge(ip); b.connect()
        groups = b.get_group()
        target_id = None
...
def cmd_demo(ip: Optional[str], name: str) -> None: # Quick loop that cycles a few hues (blue→cyan→green→yellow→red).
...
if __name__ == "__main__":
    ...
    ap.add_argument("--ip", help="Bridge IP (optional; auto-discovery if omitted)")
